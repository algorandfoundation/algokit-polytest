...
| [Invalid type](#invalid-type) | Ensure a helpful error message is thrown when giving the wrong type to a field |
...
| [decode signed transaction](#decode-signed-transaction) | A transaction with a valid signature is decoded properly |
...
Ensure a helpful error message is thrown when giving the wrong type to a field
...
A transaction with a valid signature is decoded properly
